...
Class BackendController extends Controller {
    ...
    public function userprofilesub(Request $request) {
        if (!empty($request->method()) && $request->method() == "POST") {
            ...
            $param = array('full_name' => $user_name,
        ...
        if ($request->method() == "POST") {
            ...
            if (!empty($input)) {
                if (!empty($input['banner']) && !empty($input['banner_url']) && $request->hasFile('banner_file')) {
                    ...
                    $bann = Self::getBanner($input['banner']);
                    $arrInsert = array(
                    ...
                    if (empty($bann)) {
                        ...
                        $data = DB::table($table)->where('banner_id', $input['banner'])->update($arrInsert);
                    }
    ...
    public static function getBanner($id = '') {
        ...
        $tableBannerName = config('variables.tbl_banner');
        $query = DB::table($tableBannerName)->select("$tableBannerName.*");
        $query->where($tableBannerName . '.banner_id', $id);
        return $query->get();
    }
    ...
    public function orderStatusChanged(Request $request) {
        if (!empty($request->method()) && $request->method() == "POST") {
            ...
            //var_dump($insertTerms);exit;
            //Redirect::to(url('backend/ordersList'));

            $from = config('variables.mail_from');

            $mail_id = str_replace(' ', '', $email_id);
            $arrMail = array('to' => $mail_id,
                'subject' => 'Your order have successfully dispathed from Networks Direct.',
                'from' => $from);
            ...
            $arrMailAdmin = array('to' => $from,
                ...
                'from' => $from);
            $mailResponse = \App\library\Customlibrary::order_mail_send_admin($arrMailAdmin);

            Session::flash('success', "Status Updated Successfully!");
            //return Redirect::to(url('backend/ordersList'));
        }
    }
    ...
    public function selectSubCategory(Request $request) {
        ...
        if (!empty($request->method()) && $request->method() == "POST") {
            $input = $request->input();
            $input = json_decode($input['data']);
            $data = self::fetchSubCategory($input->category_id);
            $data = (array) $data;
            if (!empty($data)) {

                foreach ($data as $sub) {
                    foreach ($sub as $sub) {
                        $html .= "<option value='$sub->subcategory_name'>" . $sub->subcategory_name . "<option>";
                        $message = 'success';
                        $status = 'success';
                    }
                }
    ...
    public function fetchSubCategory($id = '') {
        ...
        return $result = $query->get(['subcategory_name', 'subcategory_id', 'subcategory_slug']);
    }
    ...
    public function add_address(Request $request) {
        ...
        if ($request->method() == "POST") {
            ...
            if (!empty($input['address_id'])) {
                ...
                $tbl_about = config('variables.tbl_address');
                $id = $input['address_id'];
                $arrInsert = array(
                ...
                $insertTerms = DB::table($tbl_about)->where('id', $id)->update($arrInsert);
                Session::flash('success', "Page Updated Successfully!");
                return Redirect::to(url('backend/add_address'));
            } else if (!empty($input)) {
        ...
        $arrContent['aboutData'] = self::getAddressData();
        //var_dump($arrContent['aboutData']);exit;
        echo view('backend.address')->with('arrContent', $arrContent)->render();
    }
    ...
    public function add_socialMedia(Request $request) {
        ...
        if ($request->method() == "POST") {
            ...
            if (!empty($input['socialmedia_id'])) {

                $tbl_about = config('variables.tbl_social_media');
                $id = $input['socialmedia_id'];
                $arrInsert = array(
                ...
                $insertTerms = DB::table($tbl_about)->where('id', $id)->update($arrInsert);
                Session::flash('success', "Page Updated Successfully!");
                return Redirect::to(url('backend/add_socialMedia'));
            } else if (!empty($input['twitter'])) {
        ...
        $arrContent['aboutData'] = self::getSocialMediaData();
        //var_dump($arrContent['aboutData']);exit;
        echo view('backend.socialMedia')->with('arrContent', $arrContent)->render();
    }

    public function getSocialMediaData($id = '') {
        $tbl_about = config('variables.tbl_social_media');
        $query = DB::table($tbl_about)->select("$tbl_about.*");
        if (!empty($id)) {
            $query->where($table . '.sub_category', $id);
    ...
    public function allSubscribedUsers() {
        $arrContent = array();
        $arrContent['users'] = app('App\Http\Controllers\FrontendController')->getSubscriberDetail();
        echo view('backend.subscriber')->with('arrContent', $arrContent)->render();
    }
    ...
    public function deleteAllProduct($id) {
        if (!empty($id)) {
            ...
            Session::flash('success', "Deleted Products successfully!");
